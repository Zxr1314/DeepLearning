...
class ChanNet2D2(PSPnet2D2):
    ...
    def inference(self, images, **kwargs):
        ...
        x = tf.nn.dropout(x, kwargs['keep_prob'])
        output['dropout'] = x
        out_shape = [shape2[0], int(int(shape[1]) / 2), int(int(shape[2]) / 2), 256]
        x = self.conv2d_transpose(self.name + 'deconv1', x, out_shape, [2, 2, 256, 512], stride=[1, 2, 2, 1],
                                  pretrain=pretrain, train=training)
        output['deconv1'] = x
        out_shape = [shape2[0], int(int(shape[1])), int(int(shape[2])), 128]
        x = self.conv2d_transpose(self.name + 'deconv2', x, out_shape, [2, 2, 128, 256], stride=[1, 2, 2, 1],
                                  pretrain=pretrain, train=training)
        output['deconv2'] = x
        self.area = self.conv2d(self.name + 'conv6', x, [1, 1, 128, 1], pretrain=pretrain, train=training)
        output['conv6'] = self.area

        hed = self.hed.inference(res['relu3'], pretrain=pretrain, training=training)
        output['hed'] = hed
        self.edge = hed['out']
        concat = tf.concat([self.area, self.edge], axis=3, name=self.name+'concat')
        output['concat'] = concat
