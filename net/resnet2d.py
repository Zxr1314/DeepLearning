...
class ResNet2D(Net):
    ...
    def inference(self, images, **kwargs):
        ...
        for i in range(2):
            res = self.residual_empty(res['out'], 1, pad=1, lvl=2, sub_lvl=i+2, name='res_empty1_'+str(i))
            output['res_empty1_' + str(i)] = res
        # 3_1-3_3
        res = self.residual_short(res['out'], 2, pad=2, lvl=3, sub_lvl=1, name='res_short2', modify_stride=True)
        output['res_short2'] = res
        for i in range(3):
            res = self.residual_empty(res['out'], 2, pad=1, lvl=3, sub_lvl=i+2, name='res_empty2_'+str(i))
            output['res_empty2_'+str(i)] = res
        if self.layers is 50:
            res = self.residual_short(res['out'], 4, pad=2, lvl=4, sub_lvl=1, name='res_short3')
            output['res_short3'] = res
            for i in xrange(5):
                res = self.residual_empty(res['out'], 4, pad=2, lvl=4, sub_lvl=i+2, name='res_empty3_'+str(i))
                output['res_empty3_' + str(i)] = res
        elif self.layers is 101:
            res = self.residual_short(res['out'], 4, pad=2, lvl=4, sub_lvl=1, name='res_short3')['out']
            output['res_short3'] = res
            for i in xrange(22):
                res = self.residual_empty(res['out'], 4, pad=2, lvl=4, sub_lvl=i + 2, name='res_empty3_'+str(i))
                output['res_empty3_' + str(i)] = res
        ...
        # 5_1-5_3
        res = self.residual_short(res['out'], 8, pad=4, lvl=5, sub_lvl=1, name='res_short4')
        output['res_short4'] = res
        for i in xrange(2):
            res = self.residual_empty(res['out'], 8, pad=4, lvl=5, sub_lvl=i+2, name='res_empty4_'+str(i))
