...
class ResNet2D(Net):
    ...
    def residual_conv(self, input, level, pad=1, lvl=1, sub_lvl=1, modify_stride=False, pretrain=False, training=True, name=None):
        ...
        with tf.name_scope(name):
            if modify_stride:
                prev = self.conv2d(name+"reduce_"+lvl+'_'+sub_lvl, input, [1,1,input_channel,64*level], stride=[1,2,2,1], pretrain=pretrain, train=training)
                output['conv1'] = prev
            else:
                prev = self.conv2d(name+"reduce_"+lvl+'_'+sub_lvl, input, [1,1,input_channel,64*level], stride=[1,1,1,1], pretrain=pretrain, train=training)
                output['conv1'] = prev
            prev = tf.nn.relu(prev)
            output['relu1'] = prev
            prev = self.conv2d(name+"3x3_"+lvl+"_"+sub_lvl, prev, [3,3,64*level,64*level], stride=[1,1,1,1], use_bias=False, pretrain=pretrain, train=training)
            output['conv2'] = prev
            prev = tf.nn.relu(prev)
            output['relu2'] = prev
            prev = self.conv2d(name+"1x1_"+lvl+'_'+sub_lvl, prev, [1,1,64*level,256*level], stride=[1,1,1,1], use_bias=False, pretrain=pretrain, train=training)
            output['out'] = prev
        return output
    ...
    def inference(self, images, **kwargs):
        ...
        output['relu2'] = relu2
        conv3 = self.conv2d(self.name+'conv3', relu2, [3,3,64,128], use_bias=False, pretrain=pretrain, train=training)
        output['conv3'] = conv3
...
class ResNet2D2(ResNet2D):
    def __init__(self, common_params, net_params, name=None):
        super(ResNet2D2, self).__init__(common_params, net_params, name)
        return
    ...
    def inference(self, images, **kwargs):
        ...
        if 'pretrain' in  kwargs:
            ...
            pretrain = False
        conv1 = self.conv2d(self.name+'conv1', images, [3,3,input_channel,64], stride=[1,1,1,1], use_bias=False, pretrain=pretrain, train=training)
        output['conv1'] = conv1
        relu1 = tf.nn.relu(conv1, name=self.name+'relu1')
        output['relu1'] = relu1
        conv2 = self.conv2d(self.name+'conv2', relu1, [3,3,64,64], use_bias=False, pretrain=pretrain, train=training)
...
class ResNet2D3(ResNet2D):
    def __init__(self, common_params, net_params, name=None):
        super(ResNet2D3, self).__init__(common_params, net_params, name)
        return
