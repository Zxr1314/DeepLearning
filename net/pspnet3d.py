...
class PSPnet3D2(Net):
    def __init__(self, common_params, net_params, name=None):
        super(PSPnet3D2, self).__init__(common_params, net_params, name)
        self.depth = common_params['depth']
        self.resnet = ResNet3D(common_params, net_params, self.name+'res')
        return
    ...
    def build_pyramid_pooling_module(self, input, input_shape, pretrain=False, training=True):
        ...
        #res = tf.concat([input, interp_block1, interp_block2, interp_block3, interp_block6], axis=3, name='concat')
        try:
            res = tf.concat([input, interp_block1['out'], interp_block2['out']], axis=4, name=self.name+'concat')
            output['out'] = res
        except:
            res = tf.concat_v2([input, interp_block1['out'], interp_block2['out']], axis=4, name=self.name+'concat')
            output['out'] = res
        return output
