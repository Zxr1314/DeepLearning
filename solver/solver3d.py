...
class Solver3D(Solver):
    ...
    def construct_graph(self):
        ...
        self.images = tf.placeholder(tf.float32, [None, self.height, self.width, self.depth, self.channel])
        self.labels = tf.placeholder(tf.float32, [None, self.height, self.width, self.depth, self.channel])
    ...
    def forward(self, input):
        ...
        if len(input.shape) == 1:
            input.shape = [int(input.shape[0]/self.width/self.height/self.depth/self.channel), self.depth, self.height, self.width, self.channel]
        elif len(input.shape) == 4:
            input.shape = [int(input.shape[0]/self.channel), input.shape[1], input.shape[2], input.shape[3], self.channel]
        i = 0
